...
// add_custom_widget (2026-09-10 custom-widget-instancing gap fix) — file-static
// handler, same convention as MonolithUIActionsPhase2 above: kept off the
// FMonolithUIActions header so this stays a .cpp-only, Live-Coding-hot-
// compilable change (no header edit -> no forced full UBT rebuild + editor
// restart to pick up a new class-member declaration).
namespace MonolithUIActionsCustomWidget
{
    static FMonolithActionResult HandleAddCustomWidget(const TSharedPtr<FJsonObject>& Params);
}

// add_custom_widget (2026-09-10 custom-widget-instancing gap fix) — shared
// parent-resolution + slot-attach + slot-configure logic factored out of
// HandleAddWidget so HandleAddCustomWidget (which resolves its class via a
// project WidgetBlueprint's GeneratedClass instead of the native token
// allowlist) doesn't duplicate the parent/anchor/align/padding wiring.
// Behaviour is byte-for-byte identical to what HandleAddWidget did inline
// before this refactor — only the class-resolution step differs between the
// two callers.
namespace MonolithUIActionsShared
{
    static bool AttachWidgetToParent(
        UWidgetBlueprint* WBP,
        UWidget* NewWidget,
        const TSharedPtr<FJsonObject>& Params,
        UPanelWidget*& OutParentPanel,
        UPanelSlot*& OutSlot,
        FMonolithActionResult& OutError)
    {
        ...
        if (!ParentPanel)
        {
            ...
            OutError = MonolithUIInternal::MakeErrorFromSpecError(MonolithUIInternal::MakeSpecError(
                TEXT("Lookup"),
                TEXT("/parent_name"),
        ...
        if (!Slot)
        {
            ...
            OutError = FMonolithActionResult::Error(TEXT("AddChild returned null slot"));
            return false;
        }
...
void FMonolithUIActions::RegisterActions(FMonolithToolRegistry& Registry)
{
    ...
    Registry.RegisterAction(
        TEXT("ui"), TEXT("add_custom_widget"),
        TEXT("Add an instance of a project-authored custom UserWidget Blueprint (e.g. WB_EquipSlot) as a child in a Widget Blueprint's tree. "
             "Same parent/slot/positioning surface as add_widget, but resolves widget_blueprint_path via LoadObject<UWidgetBlueprint>+GeneratedClass "
             "instead of the native-only widget_class token allowlist."),
        FMonolithActionHandler::CreateStatic(&MonolithUIActionsCustomWidget::HandleAddCustomWidget),
        FParamSchemaBuilder()
            .RequiredAssetPath(TEXT("asset_path"), TEXT("Widget Blueprint asset path to add the child into"))
            .RequiredAssetPath(TEXT("widget_blueprint_path"), TEXT("Asset path of the custom WidgetBlueprint to instantiate, e.g. /Game/INVENTORY/UI/Widgets/InventoryScreen/WB_EquipSlot. A trailing '.WB_EquipSlot_C' object-path suffix is also accepted and stripped."))
            .Optional(TEXT("widget_name"), TEXT("string"), TEXT("Name for the new widget instance (auto-generated if omitted)"))
            .Optional(TEXT("parent_name"), TEXT("string"), TEXT("Parent widget name (default: root widget). Alias: parent"), { TEXT("parent") })
            .Optional(TEXT("anchor_preset"), TEXT("string"), TEXT("Anchor preset: center, top_left, stretch_fill, etc."))
            .Optional(TEXT("position"), TEXT("object"), TEXT("Canvas position: {\"x\": 0, \"y\": 0}"))
            .Optional(TEXT("size"), TEXT("object"), TEXT("Canvas size: {\"x\": 200, \"y\": 50}"))
...
FMonolithActionResult FMonolithUIActions::HandleAddWidget(const TSharedPtr<FJsonObject>& Params)
{
    ...
    // Resolve parent, attach, and apply slot/anchor/align/padding config.
    UPanelWidget* ParentPanel = nullptr;
    UPanelSlot* Slot = nullptr;
    FMonolithActionResult AttachErr;
    if (!MonolithUIActionsShared::AttachWidgetToParent(WBP, NewWidget, Params, ParentPanel, Slot, AttachErr))
    {
        return AttachErr;
    }
...
// --- add_custom_widget ---
//
// Additive sibling of add_widget: instead of resolving widget_class against
// the native-only MonolithUIInternal::WidgetClassFromName allowlist, this
// resolves a project-authored custom UserWidget Blueprint's GeneratedClass
// via LoadObject<UWidgetBlueprint> — the same asset-path -> Blueprint ->
// GeneratedClass pattern MonolithAIBehaviorTreeActions/MonolithAIControllerActions
// use to resolve Blueprint classes elsewhere in this plugin. Parent lookup,
// slot attach, and anchor/align/padding configuration are shared with
// add_widget via MonolithUIActionsShared::AttachWidgetToParent so the two
// actions stay behaviourally identical on everything except class resolution.
//
// Defined as a file-static function inside MonolithUIActionsCustomWidget
// (forward-declared near the top of this file) rather than as an
// FMonolithUIActions member, so this addition needed zero changes to
// MonolithUIActions.h — see that forward-declaration's comment for why that
// matters here (header changes force a full UBT rebuild + editor restart;
// this MCP session runs in-process inside that same editor).
namespace MonolithUIActionsCustomWidget
{
FMonolithActionResult HandleAddCustomWidget(const TSharedPtr<FJsonObject>& Params)
{
    FMonolithActionResult ParamError;
    FString AssetPath;
    if (!MonolithUIInternal::TryGetRequiredString(Params, TEXT("asset_path"), AssetPath, ParamError))
    ...
    if (!MonolithUIInternal::TryGetRequiredString(Params, TEXT("widget_blueprint_path"), WidgetBlueprintPath, ParamError))
    {
        return ParamError;
    }
    ...
    // Accept either a bare package path ("/Game/.../WB_EquipSlot") or a full
    // object path with the "_C" generated-class suffix
    // ("/Game/.../WB_EquipSlot.WB_EquipSlot_C") — strip anything from the
    // first '.' onward so LoadObject<UWidgetBlueprint> gets the package path
    // it expects (the generated CLASS lives at the object-path form; the
    // Blueprint ASSET itself is always at the bare package path).
    FString CleanPath = WidgetBlueprintPath;
    ...
    UWidgetBlueprint* CustomWBP = LoadObject<UWidgetBlueprint>(nullptr, *CleanPath);
    if (!CustomWBP)
    {
        return MonolithUIInternal::MakeErrorFromSpecError(MonolithUIInternal::MakeSpecError(
            TEXT("Asset"),
            TEXT("/widget_blueprint_path"),
            FString::Printf(TEXT("Could not load a UWidgetBlueprint at '%s'."), *CleanPath),
            TEXT("Verify the asset path and that it points at a WidgetBlueprint asset (not a regular Actor/Object Blueprint, and not a non-Blueprint asset).")));
    }

    UClass* CustomClass = CustomWBP->GeneratedClass;
    if (!CustomClass || !CustomClass->IsChildOf(UUserWidget::StaticClass()))
    {
        return MonolithUIInternal::MakeErrorFromSpecError(MonolithUIInternal::MakeSpecError(
            TEXT("Type"),
            TEXT("/widget_blueprint_path"),
            FString::Printf(TEXT("'%s' has no valid GeneratedClass (needs compilation) or its GeneratedClass is not a UUserWidget subclass."), *CleanPath),
            TEXT("Call ui::compile_widget on the target Widget Blueprint first, then retry.")));
    }

    // Guard against inserting a WBP into its own tree (or into a chain that
    // resolves back to itself) — ConstructWidget would happily build it and
    // the recursion would only surface at runtime as a stack overflow.
    if (CustomClass == WBP->GeneratedClass)
    {
        return MonolithUIInternal::MakeErrorFromSpecError(MonolithUIInternal::MakeSpecError(
            TEXT("Type"),
            TEXT("/widget_blueprint_path"),
            TEXT("Cannot add a Widget Blueprint as a child of itself — this would recurse infinitely at runtime."),
            TEXT("Target a different asset_path, or choose a different widget_blueprint_path.")));
    }
    ...
    UWidget* NewWidget = WBP->WidgetTree->ConstructWidget<UWidget>(CustomClass, WidgetFName);
    if (!NewWidget)
    {
        return FMonolithActionResult::Error(
            FString::Printf(TEXT("Failed to construct widget of class %s"), *CustomClass->GetName()));
    }

    // Resolve parent, attach, and apply slot/anchor/align/padding config —
    // identical logic to add_widget.
    UPanelWidget* ParentPanel = nullptr;
    UPanelSlot* Slot = nullptr;
    FMonolithActionResult AttachErr;
    if (!MonolithUIActionsShared::AttachWidgetToParent(WBP, NewWidget, Params, ParentPanel, Slot, AttachErr))
    ...
    TSharedPtr<FJsonObject> Result = MakeShared<FJsonObject>();
    Result->SetStringField(TEXT("widget_name"), NewWidget->GetName());
    Result->SetStringField(TEXT("widget_class"), CustomClass->GetName());
    Result->SetStringField(TEXT("widget_blueprint_path"), CleanPath);
    Result->SetStringField(TEXT("parent_name"), ParentPanel->GetName());
    Result->SetStringField(TEXT("slot_type"), Slot->GetClass()->GetName());
    Result->SetBoolField(TEXT("compiled"), bCompile);

...
void FMonolithBlueprintLayoutActions::RegisterActions(FMonolithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("blueprint"), TEXT("auto_layout"),
		...
		FParamSchemaBuilder()
			...
			.Optional(TEXT("formatter"), TEXT("string"), TEXT("Formatter: 'auto' (default, prefers Blueprint Assist if available), 'blueprint_assist' (BA only, fails if unavailable), 'vesper' (Vesper Node Cleaner only, fails if unavailable), or 'monolith' (built-in Sugiyama)"), TEXT("auto"))
			.Build());
}
...
FMonolithActionResult FMonolithBlueprintLayoutActions::HandleAutoLayout(const TSharedPtr<FJsonObject>& Params)
{
	...
	// Validate formatter
	if (Formatter != TEXT("auto") && Formatter != TEXT("blueprint_assist") && Formatter != TEXT("vesper") && Formatter != TEXT("monolith"))
	{
		return FMonolithActionResult::Error(FString::Printf(
			TEXT("Unknown formatter '%s'. Valid: 'auto', 'blueprint_assist', 'vesper', 'monolith'"), *Formatter));
	}
	...
	if (Formatter == TEXT("vesper"))
	{
		if (!IMonolithVesperFormatter::IsAvailable())
		{
			return FMonolithActionResult::Error(
				TEXT("Vesper Node Cleaner formatter is not available. "
					"Install Vesper Node Cleaner (paid Fab/Marketplace plugin) and restart the editor."));
		}

		// "selected" mode maps directly onto Vesper's node filter; "all" and
		// "new_only" both mean "format everything in the graph" here, since
		// Vesper has no concept of pinning already-placed nodes the way the
		// built-in formatter's "new_only" mode does.
		TSet<FString> NodeNameFilter;
		if (LayoutMode == TEXT("selected"))
		{
			NodeNameFilter = SelectedNodeIds;
		}

		int32 NodesFormatted = 0;
		FString ErrorMessage;
		if (!IMonolithVesperFormatter::Get().FormatNodes(Graph, NodeNameFilter, NodesFormatted, ErrorMessage))
		{
			return FMonolithActionResult::Error(FString::Printf(
				TEXT("Vesper formatter failed: %s"), *ErrorMessage));
		}

		auto Result = MakeShared<FJsonObject>();
		Result->SetStringField(TEXT("formatter_used"), TEXT("vesper"));
		Result->SetNumberField(TEXT("nodes_formatted"), NodesFormatted);
		if (LayoutMode == TEXT("new_only"))
		{
			Result->SetStringField(TEXT("warning"),
				TEXT("Vesper formatter has no 'new_only' concept; the whole graph was formatted instead."));
		}
		return FMonolithActionResult::Success(Result);
	}

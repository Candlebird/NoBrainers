...
void FMonolithBlueprintNodeActions::RegisterActions(FMonolithToolRegistry& Registry)
{
	Registry.RegisterAction(TEXT("blueprint"), TEXT("add_node"),
		...
		FParamSchemaBuilder()
			...
			.Optional(TEXT("function_name"),     TEXT("string"),  TEXT("Function name for CallFunction nodes (e.g. PrintString) or CreateDelegate nodes (the function bound as the delegate handle — resolved on this Blueprint's own class, or on target_class if given)"))
...
FMonolithActionResult FMonolithBlueprintNodeActions::HandleAddNode(const TSharedPtr<FJsonObject>& Params)
{
	...
	// ---- CreateDelegate ----
	// "Create a delegate for <Function>" pure node — binds a specific UFunction as
	// a delegate handle, typically wired into an AddDelegate/RemoveDelegate node's
	// "Delegate" input pin to bind an arbitrary function (not just a CustomEvent).
	// BUG FIX: this node type used to fall through to the generic UK2Node_
	// fallback below, which only calls AllocateDefaultPins — it never calls
	// SetFunction, so SelectedFunctionName stayed NAME_None and the node compiled
	// (or failed to compile) as an unbound, non-functional delegate. Mirror the
	// editor's own function-picker call site (SGraphNodeK2CreateDelegate::
	// OnFunctionSelected: SetFunction then HandleAnyChange(true)) so the function
	// AND its FGuid resolve correctly.
	else if (NodeType == TEXT("CreateDelegate"))
	{
		...
		{
			return FMonolithActionResult::Error(TEXT("CreateDelegate requires 'function_name'"));
		}

		// The node's Self pin is left unconnected (self-context authoring is the
		// supported use case — e.g. binding a handler function defined on this
		// same Blueprint). With an unconnected Self pin, UK2Node_CreateDelegate::
		// GetScopeClass() always resolves to the node's OWN Blueprint at runtime
		// regardless of what we search here, so scope resolution below is anchored
		// to this Blueprint's class to keep validation truthful. An explicit
		// 'target_class' is accepted only to allow a caller to double check the
		// function exists on the same class hierarchy; it is NOT wired to the Self
		// pin (that would require an additional object reference to connect).
		FString TargetClassName = Params->GetStringField(TEXT("target_class"));
		UClass* ScopeClass = BP->SkeletonGeneratedClass ? BP->SkeletonGeneratedClass : BP->GeneratedClass;
		if (!TargetClassName.IsEmpty())
		{
			UClass* Explicit = FindFirstObject<UClass>(*TargetClassName, EFindFirstObjectOptions::NativeFirst);
			if (!Explicit && !TargetClassName.StartsWith(TEXT("U")))
				Explicit = FindFirstObject<UClass>(*FString::Printf(TEXT("U%s"), *TargetClassName), EFindFirstObjectOptions::NativeFirst);
			if (!Explicit && TargetClassName.StartsWith(TEXT("U")))
				Explicit = FindFirstObject<UClass>(*TargetClassName.Mid(1), EFindFirstObjectOptions::NativeFirst);
			if (Explicit)
			{
				ScopeClass = Explicit;
			}
		...
		if (!ScopeClass)
		{
			return FMonolithActionResult::Error(TEXT(
				"CreateDelegate: Blueprint has no SkeletonGeneratedClass/GeneratedClass yet (compile it first)."));
		}

		// Resolve the function name on the scope class, with the same K2_ prefix
		// fallback used elsewhere in this file for Blueprint-callable wrappers.
		FName ResolvedFuncName(*FuncNameStr);
		if (!ScopeClass->FindFunctionByName(ResolvedFuncName) && !FuncNameStr.StartsWith(TEXT("K2_")))
		{
			const FName K2Name(*FString::Printf(TEXT("K2_%s"), *FuncNameStr));
			if (ScopeClass->FindFunctionByName(K2Name))
			{
				ResolvedFuncName = K2Name;
			}
		}
		if (!ScopeClass->FindFunctionByName(ResolvedFuncName))
		{
			return FMonolithActionResult::Error(FString::Printf(
				TEXT("Function '%s' not found on class '%s' for CreateDelegate (also tried K2_ prefix)."),
				*FuncNameStr, *ScopeClass->GetName()));
		}
		...
		Graph->AddNode(CreateDelegateNode, /*bUserAction=*/true, /*bSelectNewNode=*/false);
		CreateDelegateNode->AllocateDefaultPins();

		// The actual fix: bind the function (was previously dropped entirely).
		CreateDelegateNode->SetFunction(ResolvedFuncName);
		// NOTE: deliberately do NOT call HandleAnyChange() here. At this point the
		// node's OutputDelegate pin is freshly allocated and unconnected (the caller
		// wires it to an AddDelegate/RemoveDelegate node's Delegate pin in a
		// separate connect_pins call). HandleAnyChangeWithoutNotifying()'s IsValid()
		// check requires a connected delegate pin with a matching signature to
		// consider the node valid; when it's not yet connected, IsValid() is false
		// and the "!IsValid() && DelegatePin->LinkedTo.Num()==0" branch WIPES
		// SelectedFunctionName back to NAME_None (mirroring the editor's own
		// "user disconnected the delegate pin, clear the stale name" behavior) —
		// which silently discarded the very function we just set. Resolve the Guid
		// by hand instead (same lookup HandleAnyChangeWithoutNotifying would do on
		// success) and leave full validation to fire naturally once the caller
		// connects the OutputDelegate pin: UK2Node_CreateDelegate::
		// PinConnectionListChanged() calls HandleAnyChange() itself on that
		// connection, and by then the pin has a real signature to validate against.
		UBlueprint::GetGuidFromClassByFieldName<UFunction>(ScopeClass, ResolvedFuncName, CreateDelegateNode->SelectedFunctionGuid);

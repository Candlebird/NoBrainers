...
**UPDATE (2026-09-24) — day/night cycle redesigned, superseding the pacing above (user-confirmed 2026-09-24):** Zombie spawning is no longer wave-based (spawn-all-then-wait-for-clear); `BP_ZombieSpawnerManager` now spawns continuously at random intervals throughout the entire Night phase, and Night ends purely on its timer — zombies already spawned are never forcibly despawned and can persist into later phases. Two new phases were added, so the cycle is now **Night** (zombies spawn) → **Morning** (nothing spawns) → **Day** (customers spawn) → **Dusk** (customers stop, zombies still don't spawn) → back to **Night**, repeating indefinitely; `E_GamePhase` gained `Morning`/`Dusk` enumerators. `BP_GameMode_ZombieStore` gained `MorningPhaseDuration`/`DuskPhaseDuration` (both default 60.0) alongside the existing `DayPhaseDuration`/`NightPhaseDuration` (both now default 300.0, up from 75.0/120.0 above), and `TargetDayToWin` dropped from 10 to 9. Build Mode is now allowed during Morning and Dusk in addition to Day (blocked only in Night/RunOver), and `CloseShopEarly` from Day now routes to Dusk (not straight to Night). At these new defaults a full win-run is ~108 minutes, exceeding this task's 15–45 min target band — a conscious, user-confirmed deviation accepted in exchange for a real timer-driven Night (no more zombie-count gate) and readable four-phase pacing. `PhaseTimeRemaining` still has no HUD consumer; **needs manual PIE verification** (see automation coverage below for the phase-order/timer logic itself).
...
* [x] **Defeat State:** Triggered when all connected players are dead. (Already implemented pre-existing in `CheckRunOverCondition` -> `EndRun(false)`; not built as part of this task. **UPDATE (2026-09-24, user-confirmed):** widened from Night-only to any phase except RunOver, since zombies spawned during Night can now persist into Morning/Day/Dusk and keep attacking — `CheckRunOverCondition` changed from `CurrentPhase==Night` to `CurrentPhase!=RunOver`.)

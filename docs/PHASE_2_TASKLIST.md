...
- **STATUS NOTE (2026-09-24, later overnight — customer/crate flow toggle, IN PROGRESS / NOT YET COMMITTED):** added `bShippingCrateOnly` (bool, replicated, default `False`) to `BP_GameState_ZombieStore`, per the design intent that the Shipping Crate (task 4.3) is a fallback for when customer-NPC assets aren't ready — flipping this flag should let the store run on crates alone with no customer spawning. `BP_CustomerSpawner::TrySpawnCustomer()` is gated on it (skips spawning when true); `ue-architect` reviewed the graph and confirmed the branch logic/polarity is correct, with two known limitations, both still open: (1) flipping the flag back to `false` mid-day does not resume spawning — the spawn timer only re-arms on the next `StartSpawning` call, so treat this as a session-level setting for now, not a live toggle; (2) `BP_GameState_ZombieStore` is GameMode-spawned, not level-placed, so there is currently no in-level or in-game way to flip the flag — only the class default. Giving it a real on/off switch (console command or GameMode-default-copied-to-GameState) is still undecided and, per the architect, should go through a proper architect pass as its own small feature rather than being bolted on here.
  - **Also in flight, also not yet committed:** a new automation test `Test_ShippingCrate_LiquidateAll_HandlesMultipleCrates` (covers `LiquidateAllCrates()` across multiple crates, the actual day-end auto-sell path — previously only single-crate liquidation was tested) and `Test_CustomerSpawner_ShippingCrateOnlyBlocksSpawn` (asserts the toggle actually blocks spawning). Landing these surfaced a **test-timing race**, not a gameplay bug: two async, `Delay`-chained crate tests were mutating the same shared crate/`StoreCash` concurrently, causing spurious cash-mismatch failures. First fix pass (re-sequencing the two crate tests so one only starts after the other's delayed check finishes) got the original `Test_ShippingCrate_LiquidatesToStoreCash` passing again, but a subsequent full-suite run came back **inconsistent and still not clean**: the multi-crate test itself then failed, a previously-unrelated test (`Test_DayEndAutoSellCustomerItems`) failed for the first time, the new toggle test didn't log a result at all, and the test-runner flagged its own log read as incomplete (19 unique tests seen vs. 26 expected) — so it's unclear how much of that result is a real second-order timing collision (same class of bug, different pair of async tests) versus a bad/truncated log read.
  - **Next session should:** get one clean, trustworthy full-suite run first (re-run with a careful log read before concluding anything is actually broken); if `Test_ShippingCrate_LiquidateAll_HandlesMultipleCrates` and/or `Test_DayEndAutoSellCustomerItems` genuinely fail on a clean run, treat it as the same shared-state/async-timing class of issue as before (consider giving cash-mutating async tests their own isolated crates/state rather than chaining them back-to-back). Only commit (`BP_GameState_ZombieStore`, `BP_CustomerSpawner`, `BP_TestController`) once the suite is verified green.

...
[ ] 5.3 Purchase Execution Logic — **BLOCKED, partial progress committed**

Validate player cash reserves on server before completing purchase.

- **Blocked on a Monolith tool limitation, not a design question.** `BP_GameState_ZombieStore` additions from the architect plan (`KioskPurchaseCounts` map, `GetKioskPurchaseCount`, `Server_RecordKioskPurchase`, `Server_AddAdvertisementLevel`) are complete and saved. On `BP_PlayerController_ZombieStore`, `CanFulfillKioskEntry` and `FulfillKioskEntry` are complete and compile clean. But `ExecuteKioskPurchase` and the two Custom Events (`Server_RequestKioskPurchase`, `Client_KioskPurchaseResult`) are stubs only: Monolith (plugin v0.22.0) has no action that can add typed input/output pins to an already-created `K2Node_CustomEvent` — confirmed exhaustively (`set_function_params` rejects custom events, `add_event_node`'s schema has no params field, no `set_custom_event_params` equivalent exists anywhere in the 129 blueprint-namespace actions). A parameterized Server/Client RPC in pure Blueprint can only exist as a Custom Event with user-defined pins, so there is no tool-side workaround — this needs a human to open `BP_PlayerController_ZombieStore`'s EventGraph and, via the Details panel's "+" button, add: `Server_RequestKioskPurchase` → input `RowName : Name`; `Client_KioskPurchaseResult` → inputs `bSuccess : Boolean`, `Message : Text`. Once those pins exist, the remaining wiring (`ExecuteKioskPurchase`'s 9-step body, both events' bodies, and `WBP_KioskCatalog::RequestPurchase`'s rewrite + new `ShowPurchaseResult` function) can resume immediately — the logic patterns are already proven out in the completed `CanFulfillKioskEntry`/`FulfillKioskEntry` functions.
- **Needs manual editor action (not testing — this blocks further automated progress):** add the two RPC parameter sets described above to `BP_PlayerController_ZombieStore`'s `Server_RequestKioskPurchase`/`Client_KioskPurchaseResult` Custom Events, then this task can be resumed and finished.

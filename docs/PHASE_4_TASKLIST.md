...
- Known minor issue: see `docs/BUGS.md` — "Decorative barrel actor has auto-generated name / no outliner folder."
...
- Night phase ending on a fixed timer → fixed. Night phase now ends only when the zombie count hits zero rather than after a fixed duration. Committed `c9e30e8`. **SUPERSEDED (2026-09-24, user-confirmed):** the day/night redesign (see `docs/PHASE_5_TASKLIST.md`'s 2026-09-24 update) reverted this — Night once again ends purely on a timer (`NightPhaseDuration`), since zombie spawning is now continuous and open-ended rather than a finite wave, and zombies are no longer despawned or gated on at Night's end.
...
- Night-phase timer-based end condition (see the 2026-09-24 SUPERSEDED note above — zombie-count-based ending was reverted), including continuous zombie spawning throughout Night and zombies persisting into Morning/Day/Dusk.
